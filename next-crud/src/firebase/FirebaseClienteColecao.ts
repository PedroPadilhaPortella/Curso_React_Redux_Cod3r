...
export default class FirebaseClienteColecao implements ClienteRepositorio {
  ...
  private colecaoCliente = collection(dataBase, 'clientes').withConverter(this.conversor)

  async salvar(cliente: Cliente): Promise<Cliente> {
    if (cliente?.id) {
      await setDoc(doc(dataBase, 'clientes', cliente.id).withConverter(this.conversor), cliente);
      return cliente;
    } else {
